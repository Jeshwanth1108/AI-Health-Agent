...
class Agent:
    def __init__(self, medical_report=None, role=None, extra_info=None):
        ...
        # Initialize the prompt template based on role and info
    ...
    def create_prompt_template(self):
        if self.role == "MultidisciplinaryTeam":
            # The multidisciplinary template uses extra_info reports directly, no placeholders needed
            templates = f"""
            ...
            # For other roles, use placeholders {medical_report} to be filled in when formatting
            templates_dict = {
                "Cardiologist": """
            ...
            templates = templates_dict[self.role]

        # Return a LangChain prompt template instance
        return PromptTemplate.from_template(templates)

    def run(self):
        print(f"{self.role} is running...")
        # Format prompt with medical report (only for single-role agents)
        if self.role == "MultidisciplinaryTeam":
            # Multidisciplinary already has filled info, no formatting needed
